...
export class FormUpdateCreateScheduleComponent implements OnInit {
  ...
  public unenabled_hour_morning: boolean = false
  public unenabled_hour_afternoon: boolean = false
  public unenabled_hour_night: boolean = false
  public unenabled_hour_edit: boolean = false
  public edit: boolean = false;
  public editAllSchedule: boolean = false;
  public scheduleEdit: ScheduleCrud;
  ...
  public initItemsRoomsPavilionSelected(element) {
    this.subscription.push(
      this.roomService.readRoomByIdPavilion(element.value).subscribe(res => {
        this.rooms = res;
  ...
  public onSubmit(form): void{
    ...
    if(this.edit) {
      ...
      console.log(this.scheduleEdit)
      this.subscription.push(
        this.scheduleService.updateSchedule(this.scheduleEdit).subscribe((res) => {
      ...
      if (this.editAllSchedule) {
        this.subscription.push(
          this.scheduleService.updateAllSchedule(this.schedule).subscribe((res) => {
            this.router.navigate(['homeSchedule/list']); 
          }) 
        )
      } else {
  ...
  public getIdScheduleRoom(idRoom): void {
    // FAZER CONSULTA; CASO RETORNO O id_schedule, FAZER UM UPDATE; CASO RETORNE null, CRIAR O HORÁRIO
  
    if (this.schedule.fk_id_room === null){
      this.messageService.openSnackBar('Selecione o pavilhão e sala!', 'alertMessage');   
    }else{
      this.scheduleService.getIdScheduleRoom(this.schedule.fk_id_room, this.getShiftTabGroup()).subscribe((res: Schedule) => {
        
        console.log(res[0])
        
        if (res[0].shift_schedule != null) {
          ...
          this.schedule.id_schedule = res[0].id_schedule   
          this.schedule.active_schedule = true     
          
          this.setShiftObjSchedule(res[0].shift_schedule);
